...
## Changelog 2026-05-10 — sync với API.md

BE đã align xong toàn bộ Section 17 checklist (23/23 ✅). Các điểm FE đã update spec:

| # | Thay đổi | Section ảnh hưởng |
|---|---|---|
| ✅ | Customer hold TTL = **24 giờ** (BE business rule, KHÔNG phải 15 phút) | §15.1, Glossary |
| ✅ | Staff hold TTL = **30 phút** | Glossary |
| ✅ | Phone regex `^0\d{9}$` (10 số bắt đầu 0) — FE PhoneInput đã match | §14.6 |
| ✅ | Apple email có thể là `xyz@privaterelay.appleid.com` — accept như email thường | §4.4 |
| ✅ | `/auth/forgot-password` rate limit per-IP (5/h) — không phải per-email | OK |
| ✅ | `/bookings/calendar/:propertyId?year=&month=` — FE đã gọi đúng | §6.5 |

> Mọi endpoint được liệt kê trong APP_SPEC đều đã LIVE trên `https://api.halong24h.com`.
...
[Customer App] /home → /search → tap property → /rooms/:id
                                            ...
                                            POST /bookings/customer-hold
                                              { propertyId, checkinDate, checkoutDate, guestCount, ... }
                                                    │
                                                    ▼ 201 Created
                                            Booking với status=HOLD
                                            holdExpireAt = now + 24h (customer hold)
                                                    │
...
| **Hold** | Booking tạm giữ phòng. Staff hold = 30 phút, Customer hold = 24 giờ. FE đọc `holdRemainingSeconds` từ BE để countdown |
