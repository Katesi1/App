...
4. **3 cổng khoá tạo phòng (theo thứ tự):** (a) OWNER phải có **SĐT trong hồ sơ** (v1.41 — BE 403 `code:"PHONE_REQUIRED"` ở `POST/PUT/PATCH /properties` + `POST /staff/invites` nếu `phone` rỗng), (b) phải **KYC approved**, rồi (c) phải có **bank account approved** (`hasApprovedBank`). SALE/ADMIN không bị các cổng này. FE bắt `PHONE_REQUIRED` → điều hướng `/profile/edit` cập nhật SĐT rồi thử lại (không cần re-login); ngoài ra pre-check chủ động trước khi mở form.
...
0. **SĐT bắt buộc** (v1.41) — `ensureOwnerHasPhone`: OWNER `phone` rỗng → dialog "Cần số điện thoại" → `/profile/edit`. BE cũng chặn ở server (403 `PHONE_REQUIRED`) nếu client bỏ qua.
...
SALE/ADMIN không qua các cổng này (SALE bị chặn tạo property hoàn toàn).

> **Trial cap (v1.39):** OWNER trial ngầm chưa mua gói chỉ đăng tối đa 1 cơ sở. Đăng cơ sở thứ 2 → BE 403 `code:"PROPERTY_LIMIT_REACHED"`, message trung tính; FE hiện toast + CTA "Liên hệ hỗ trợ" (`/profile/help`).
...
- [ ] 3 cổng tạo phòng: SĐT (PHONE_REQUIRED) → KYC approved → bank approved. Trial cap 1 cơ sở (PROPERTY_LIMIT_REACHED).
